...
if (camila_form_in_update_mode(segreteriacampo_worktable1)) {

    

    $form = new dbform('segreteriacampo_worktable1', 'id');
    ...
    new form_textbox($form, 'nominativo', 'Nominativo', true, 30, 255, 'uppercase');

    
    new form_textbox($form, 'titolo', 'Titolo', false, 30, 255, '');

    
    new form_textbox($form, 'gruppoorganizzazioneente', 'Gruppo/Organizzazione/Ente', false, 30, 255, '');

    
    new form_textbox($form, 'ruolo', 'Ruolo', false, 30, 255, '');

    
    new form_phonenumber($form, 'cellulare', 'Cellulare', false, 30, 255, '');

    
    new form_textbox($form, 'email', 'E-Mail', false, 30, 255, '');

    
    new form_phonenumber($form, 'telfisso', 'Tel. Fisso', false, 30, 255, '');

    
    new form_phonenumber($form, 'fax', 'Fax', false, 30, 255, '');

    
    new form_phonenumber($form, 'altrotelefono', 'Altro telefono 1', false, 30, 255, '');

    
    new form_phonenumber($form, 'altrotelefono1', 'Altro telefono 2', false, 30, 255, '');
...
{
$form->fields['nominativo']->autosuggest_table = 'segreteriacampo_worktable18';
...
$form->fields['nominativo']->autosuggest_pickfields = 'cellulare';
$form->fields['nominativo']->autosuggest_destfields = 'cellulare';
}
...
$form->fields['cellulare']->autosuggest_pickfields = 'cognome';
$form->fields['cellulare']->autosuggest_destfields = 'nominativo';
}
    ...
    $form->draw();
      ...
      if ($_CAMILA['page']->camila_exporting())
          ...
          $mapping = 'created=Data creazione#last_upd=Ultimo aggiornamento#last_upd_by=Utente ult. agg.#last_upd_src=Sorgente Ult. agg.#last_upd_by_name=Nome Utente ult. agg.#last_upd_by_surname=Cognome Utente ult. agg.#mod_num=Num. mod.#id=Cod. riga#created_by=Utente creaz.#created_src=Sorgente creaz.#created_by_surname=Cognome Utente creaz.#created_by_name=Nome Utente creaz.#cf_bool_is_special=contrassegnati come speciali#cf_bool_is_selected=selezionati#nominativo=Nominativo#titolo=Titolo#gruppoorganizzazioneente=Gruppo/Org./Ente#ruolo=Ruolo#cellulare=Cellulare#email=E-Mail#telfisso=Tel. Fisso#fax=Fax#altrotelefono=Altro telefono 1#altrotelefono1=Altro telefono 2#note=Note';
	  ...
	  //if ($_CAMILA['adm_user_group'] == CAMILA_ADM_USER_GROUP)
	//	  $stmt = 'select ' . $admin_report_fields . ' from segreteriacampo_worktable1';
	  //else
		  $stmt = 'select ' . $report_fields . ' from segreteriacampo_worktable1';
      
      $report = new report($stmt.$filter, '', 'nominativo', 'asc', $mapping, null, 'id', $default_fields, '', (isset($_REQUEST['camila_rest'])) ? false : true, (isset($_REQUEST['camila_rest'])) ? false : true);
