...
if (camila_form_in_update_mode('segreteriacampo_worktable12')) {

    

    $form = new dbform('segreteriacampo_worktable12', 'id');
...
if (is_object($form->fields['codicebadge'])) $form->fields['codicebadge']->defaultvalue = worktable_parse_default_expression('${prefissocodiceabarre}${codice riga}', $form);
    ...
    $form->draw();
      ...
      $report_fields = 'id,cf_bool_is_special,cf_bool_is_selected,nome,cognome,organizzazioneentesocieta,accessodal,accessoal,codicebadge,motivazione,note,created,created_by,created_by_surname,created_by_name,last_upd,last_upd_by,last_upd_by_surname,last_upd_by_name,mod_num';
	  //$admin_report_fields = '';
      $default_fields = 'cf_bool_is_special,cf_bool_is_selected,nome,cognome,organizzazioneentesocieta,accessodal,accessoal,codicebadge,motivazione,note';
      ...
      if ($_CAMILA['page']->camila_exporting())
          $mapping = 'created=Data creazione#last_upd=Ultimo aggiornamento#last_upd_by=Utente ult. agg.#last_upd_src=Sorgente Ult. agg.#last_upd_by_name=Nome Utente ult. agg.#last_upd_by_surname=Cognome Utente ult. agg.#mod_num=Num. mod.#id=Cod. riga#created_by=Utente creaz.#created_src=Sorgente creaz.#created_by_surname=Cognome Utente creaz.#created_by_name=Nome Utente creaz.#cf_bool_is_special=contrassegnati come speciali#cf_bool_is_selected=selezionati#nome=NOME#cognome=COGNOME#organizzazioneentesocieta=ORGANIZZAZIONE/ENTE/SOCIETA\'#accessodal=ACCESSO DAL#accessoal=ACCESSO AL#codicebadge=CODICE BADGE#motivazione=MOTIVAZIONE#note=NOTE';
      else
          $mapping = 'created=Data creazione#last_upd=Ultimo aggiornamento#last_upd_by=Utente ult. agg.#last_upd_src=Sorgente Ult. agg.#last_upd_by_name=Nome Utente ult. agg.#last_upd_by_surname=Cognome Utente ult. agg.#mod_num=Num. mod.#id=Cod. riga#created_by=Utente creaz.#created_src=Sorgente creaz.#created_by_surname=Cognome Utente creaz.#created_by_name=Nome Utente creaz.#cf_bool_is_special=contrassegnati come speciali#cf_bool_is_selected=selezionati#nome=NOME#cognome=COGNOME#organizzazioneentesocieta=ORG./ENTE/SOC.#accessodal=ACCESSO DAL#accessoal=ACCESSO AL#codicebadge=CODICE BADGE#motivazione=MOTIVAZIONE#note=NOTE';
	  ...
	  //if ($_CAMILA['adm_user_group'] == CAMILA_ADM_USER_GROUP)
	//	  $stmt = 'select ' . $admin_report_fields . ' from segreteriacampo_worktable12';
	  //else
		  $stmt = 'select ' . $report_fields . ' from segreteriacampo_worktable12';
      
      $report = new report($stmt.$filter, '', 'organizzazioneentesocieta', 'asc', $mapping, null, 'id', $default_fields, '', (isset($_REQUEST['camila_rest'])) ? false : true, (isset($_REQUEST['camila_rest'])) ? false : true);
