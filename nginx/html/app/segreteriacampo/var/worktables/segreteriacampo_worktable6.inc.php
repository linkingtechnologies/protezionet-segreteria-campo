...
if (camila_form_in_update_mode(segreteriacampo_worktable6)) {

    

    $form = new dbform('segreteriacampo_worktable6', 'id');
    ...
    new form_integer($form, 'numerosegnalazione', 'NUMERO SEGNALAZIONE', false, 5, 255, '');
if (is_object($form->fields['numerosegnalazione'])) $form->fields['numerosegnalazione']->defaultvalue = worktable_get_next_autoincrement_value('segreteriacampo_worktable6','numerosegnalazione');
if ($_CAMILA['adm_user_group'] != CAMILA_ADM_USER_GROUP && is_object($form->fields['numerosegnalazione'])) $form->fields['numerosegnalazione']->updatable = false;
...
if (is_object($form->fields['data1'])) $form->fields['data1']->defaultvalue = date('Y-m-d');
    ...
    new form_textbox($form, 'mezzoattrezzatura', 'MEZZO/ATTREZZATURA', true, 30, 255, '');
...
$form->fields['codicecurvo']->autosuggest_destfields = 'cognome,nome,organizzazione,cellulare';
    ...
    $form->draw();
	  ...
	  //if ($_CAMILA['adm_user_group'] == CAMILA_ADM_USER_GROUP)
	//	  $stmt = 'select ' . $admin_report_fields . ' from segreteriacampo_worktable6';
	  //else
		  $stmt = 'select ' . $report_fields . ' from segreteriacampo_worktable6';
      
      $report = new report($stmt.$filter, '', 'numerosegnalazione', 'desc', $mapping, null, 'id', $default_fields, '', (isset($_REQUEST['camila_rest'])) ? false : true, (isset($_REQUEST['camila_rest'])) ? false : true);
...
$report->menuitems[]=$jarr;
$jarr=Array();
$jarr['url'] = "javascript:camila_inline_update_selected('mezzoattrezzatura','')";
$jarr['visible'] = 'yes';
$jarr['short_title'] = 'MODIFICA MEZZO/ATTREZZATURA...';
$jarr['parent'] = 'index.php';
$report->menuitems[]=$jarr;
$jarr=Array();
