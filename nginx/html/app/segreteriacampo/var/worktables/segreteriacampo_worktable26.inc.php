...
if (camila_form_in_update_mode(segreteriacampo_worktable26)) {

    

    $form = new dbform('segreteriacampo_worktable26', 'id');
...
if (is_object($form->fields['dataora'])) $form->fields['dataora']->defaultvalue = date('Y-m-d H:i:s');
    ...
    new form_textarea($form, 'descrizione', 'Descrizione', true, 10, 80, 255, '');
    ...
    $form->draw();
      ...
      $report_fields = 'id,cf_bool_is_special,cf_bool_is_selected,dataora,descrizione,created,created_by,created_by_surname,created_by_name,last_upd,last_upd_by,last_upd_by_surname,last_upd_by_name,mod_num';
	  //$admin_report_fields = '';
      $default_fields = 'cf_bool_is_special,cf_bool_is_selected,dataora,descrizione';
	  ...
	  //if ($_CAMILA['adm_user_group'] == CAMILA_ADM_USER_GROUP)
	//	  $stmt = 'select ' . $admin_report_fields . ' from segreteriacampo_worktable26';
	  //else
		  $stmt = 'select ' . $report_fields . ' from segreteriacampo_worktable26';
